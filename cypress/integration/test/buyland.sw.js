...
describe('Log in to Superworld', () => {
  ...
  it('type into an element to search for plots of land', () => {
    ...
    for(let n = 0; n < 7; n ++) {
      cy.get('button.mapboxgl-ctrl-icon.mapboxgl-ctrl-zoom-in')
        .click()
      }
  })

  it('zoomed to land to buy now', () => {
    //Search for plots of land
     cy.get('#view-MapView').click(500, 500, { multiple: true })
     cy.get('.LoginButton-header').click({force: true})
  })
